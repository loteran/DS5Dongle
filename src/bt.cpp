...
static void __not_in_flash_func(hci_packet_handler)(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    switch (event_type) {
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            // Do NOT call tud_disconnect() here (unlike upstream, which hides the USB
            // device when nothing is paired): disconnecting the full USB device also
            // tears down the UAC1 audio interface, which causes PipeWire to lose
            // ds5_dongle_sink and disrupts headset/haptics audio routing on the host.
            // USB stays connected; the host's audio loopback keeps working while we
            // search for the controller again via inquiry.
            gap_connectable_control(1);
            gap_discoverable_control(1);
            ...
            // Drop any packets still queued for the connection that just went away,
            // so they don't get sent stale to the next one.
            ...
            // Auto-restart inquiry so an existing controller that dropped out
            // reconnects without needing the BOOTSEL-button scan trigger below.
            gap_inquiry_start(30);
            break;
        }
